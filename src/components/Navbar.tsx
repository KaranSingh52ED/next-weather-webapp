    ...
    if (value.length >= 3) {
      try {
        const response = await axios.get(
          `https://api.openweathermap.org/data/2.5/find?q=${value}&appid=658767b0ae936b022f59a69f44868419`
        );
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (postiion) => {
        ...
        try {
          setLoadingCity(true);
          const response = await axios.get(
            `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=658767b0ae936b022f59a69f44868419`
          );
          setTimeout(() => {
            setLoadingCity(false);
        ...
        <div className="h-[80px]     w-full    flex   justify-between items-center  max-w-7xl px-3 mx-auto">
          ...
          <section className="flex gap-2 items-center">
            <MdMyLocation
              ...
              className="text-2xl  text-red-500 hover:opacity-80 cursor-pointer"
            />
  ...
  return (
    <>
      {((showSuggestions && suggestions.length > 1) || error) && (
        <ul className="mb-4 absolute border top-[44px] left-0 border-blue-700 bg-purple-200 rounded-md min-w-[200px] shadow-md shadow-black z-1  flex flex-col gap-1 py-2 px-2">
